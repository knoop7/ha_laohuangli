...
class AlmanacDevice:
    ...
    def get_holiday(self,date_str,cnlunar_holidays): return next((v for d in(self._holiday_cache,self._workday_cache)if date_str in d for v in[d[date_str]]),cnlunar_holidays or"暂无节日")

class UpdateManager:
    def __init__(self):
        self._last_update,self._lock,self._update_counts,self._cleanup_task={},asyncio.Lock(),defaultdict(int),None
    async def start(self):
        if not self._cleanup_task:self._cleanup_task=asyncio.create_task(self._periodic_cleanup())
    async def _periodic_cleanup(self):
        while True:
            await asyncio.sleep(3600)
            async with self._lock:self._update_counts.clear()
    async def stop(self):
    ...
    async def can_update(self,sensor_type):
        ...
        if not last or(datetime.now()-last)>=timedelta(seconds=5):self._last_update[sensor_type]=datetime.now();self._update_counts[sensor_type]+=1;return True
        return False

class AlmanacSensor(SensorEntity):
    _shared_lunar_cache,_cache_timestamps,_MAX_CACHE_SIZE,_CACHE_TTL=WeakValueDictionary(),{},24,3600
    _last_memory_check,_MEMORY_CHECK_INTERVAL=0,300
    ...
    async def _get_lunar_data(cls,date):
        ...
        async with cls._cache_lock:
            [cls._shared_lunar_cache.pop(k,None)or cls._cache_timestamps.pop(k,None)for k in list(cls._cache_timestamps)if(now-cls._cache_timestamps[k]).total_seconds()>cls._CACHE_TTL]
            if key in cls._shared_lunar_cache:cls._cache_timestamps[key]=now;return cls._shared_lunar_cache[key]
            try:data=cnlunar.Lunar(date,godType='8char');cls._shared_lunar_cache[key]=data;cls._cache_timestamps[key]=now;return data
    ...
    async def async_update(self):
        ...
        except Exception as e: _LOGGER.error(f"更新传感器时出错 {self._type}: {e}");self._available=False

    async def cleanup(self):
        try:
            self._cleanup_called=True
            async with self._update_lock:
                self._attributes.clear();self._state=None
                self._device=self._hass=self._text_processor=self._time_helper=None
        except Exception as e:_LOGGER.error(f"清理时出错: {e}")
    ...
    async def _update_general(self,now):
        try:
            lunar_data=await self._get_lunar_data(now)
            if not lunar_data: self._available=False;return
            ...
            self._available=True;return self._state
        except:self._available=False;return None

async def setup_sensor_updates(hass,sensors,um):
    UK="almanac_unsubs"
    if DOMAIN in hass.data and UK in hass.data[DOMAIN]:[unsub() for unsub in hass.data[DOMAIN][UK]];hass.data[DOMAIN][UK]=[]
    async def process_updates(stu,gt):
        async with um._lock:
            if await um.can_update(gt):[await s.async_update() or s.async_write_ha_state() for s in stu if not s._updating and not s._cleanup_called]
    @callback
    def _async_update(sensors_group,group_type):hass.async_create_task(process_updates(sensors_group,group_type))
    @callback
    def create_update_handler(sensors_group,group_type):
        @callback
        def handler(*_):
            if dt.as_utc(dt.now())<=dt.now()+timedelta(hours=12):_async_update(sensors_group,group_type)
        return handler
    sg={'shichen':[s for s in sensors if s._type=='时辰'],'other':[s for s in sensors if s._type not in ['日期']]}
    ut={'shichen':{'second':0,'minute':[0,15,30,45]},'other':{'second':0,'minute':0,'hour':'*'}}
    for group_type,sensors_group in sg.items():
        if sensors_group:
            handler=create_update_handler(sensors_group.copy(),group_type)
            hass.data.setdefault(DOMAIN,{}).setdefault(UK,[]).append(async_track_time_change(hass,handler,**ut[group_type]))
...
async def setup_almanac_sensors(hass,eid,cd):
    ...
    d=AlmanacDevice(eid,cd.get("name","中国老黄历"));um=UpdateManager();await um.start()
