...
class BirthdaySensor(SensorEntity):
    def __init__(self, hass, device, person, sensor_type, entry_id: str):  
        self.hass = hass
        self._device = device
        self._entry_id = entry_id  
        self._person = person
        ...
        self._attr_unique_id = f"{self._entry_id}_{self._person['name']}_{self._type}"
        
        if self._type in ["星座", "今日穿搭", "今日运势", "生存天数", "周岁"]:
...
async def setup_birthday_sensors(hass: HomeAssistant, entry_id: str, config_data: dict):
    _LOGGER.debug("Setting up birthday sensors with entry_id: %s", entry_id)
