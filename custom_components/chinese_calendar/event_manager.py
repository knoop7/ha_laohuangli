...
class EventSensor(SensorEntity):
    ...
    def extra_state_attributes(self):
        ...
        if not self._event_date:
            base_attrs = {
                self._convert_text("描述"): self._event_desc,
                self._convert_text("自动删除"): self._convert_text("开启" if self._auto_remove else "关闭"),
                self._convert_text("完整倒计时"): self._convert_text("开启" if self._full_countdown else "关闭"),
                self._convert_text("通知服务"): self._notification_service if self._notification_service else self._convert_text("关闭")
            }
        else:
            base_attrs = {
                self._convert_text("描述"): self._event_desc,
                self._convert_text("日期"): self._event_date.strftime(EVENT_DATE_FORMAT),
                self._convert_text("自动删除"): self._convert_text("开启" if self._auto_remove else "关闭"),
                self._convert_text("完整倒计时"): self._convert_text("开启" if self._full_countdown else "关闭"),
                self._convert_text("通知服务"): self._notification_service if self._notification_service else self._convert_text("关闭")
            }
            base_attrs.update(self._attributes)
        ...
        for key, value in base_attrs.items():
            if isinstance(value, str):
                ...
                for k, v in value.items():
                    if isinstance(v, str):
                        converted_dict[self._convert_text(k)] = self._convert_text(v)
                    else:
                        converted_dict[self._convert_text(k)] = v
    ...
    def _format_countdown(self, delta):
        if not self._full_countdown:
            return self._convert_text(f"还有{delta.days}天")
        ...
        if days > 0:
            return self._convert_text(f"还有{days}天")
        
        parts = []
        if hours > 0:
            parts.append(f"{hours}{self._convert_text('时')}")
        if minutes > 0:
            parts.append(f"{minutes}{self._convert_text('分')}")
        if seconds > 0 or not parts:  
            parts.append(f"{seconds}{self._convert_text('秒')}")
            ...
            if self._auto_remove and delta.days < -1:
                ...
                if self._registry and self._entity_id:
                    try:
                        ...
                        _LOGGER.error(f"删除实体失败: %s", e)
                self._available = False
                return

            if total_seconds <= -86400:  
                self._state = self._convert_text("已过期")
            elif self._full_countdown:
                if -3600 < total_seconds <= 0:
                    self._state = self._convert_text("已到时间")
                ...
                if delta.days == 0:
                    self._state = self._convert_text("今天")
                    await self._handle_notification()
                elif total_seconds <= 0:
                    self._state = self._convert_text("已过期")
                else:
    ...
    async def _handle_notification(self):
        if (self._notification_service and 
            ...
            try:
                service_data = {
                    "title": self._convert_text("中国老黄历 · Home Assistant"),
                    "message": self._convert_text(self._notification_message)
                }
...
async def setup_event_sensors(hass: HomeAssistant, entry_id: str, config_data: dict):
    ...
    event_device = EventDevice(entry_id, language)
    event_count = 1
