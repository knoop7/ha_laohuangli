...
window.customCards.push({
  ...
  preview: false,
  documentationURL: "https://github.com/knoop7/almanac-card"
});
...
class AlmanacCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this.config = {
      ...config,
      show_yiji: config.show_yiji ?? false,
      show_festivals: config.show_festivals ?? false,
      show_datetime: config.show_datetime ?? false,
  ...
  _handleDateControl(action, date = null) {
    if (this._hass) {
      ...
      this._hass.callService("chinese_calendar", "date_control", serviceData);
    }
  }
